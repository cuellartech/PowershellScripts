...
[string]$SiteServer = "spsccm.central.pima.gov"
[string]$SiteCode = "PIM"
...
foreach ($DistributionPoint in $DistributionPoints) {
    ...
    if ( -not(Test-Connection $DistributionPointName -Quiet -Count 1)) {
        ...
        if (($CurrentPackageList.Count -eq 0) -or ($null -eq $CurrentPackageList)){
            ...
            if ($InvalidPackages.Count -eq 0){
                ...
                if ($InvalidPackages.Count -gt $WarnThreshold) {
                    ...
                    $response = Suspend-Script -Message "Proceed with removing invalid WMI packages on $($Env:COMPUTERNAME)?" -Warning -Buttons 4
                } else {
    ...
    Invoke-Command -ComputerName $DistributionPointName -ScriptBlock {
        ...
        $dataDrives = @(Get-PSDrive -PSProvider FileSystem | Where-Object{$_.Used -gt 0})
        ...
        if ($null -eq $selectedDrive) {
            ...
            $pkgs = @()
            Get-ChildItem -Path $path | Select-Object Name,Basename | ForEach-Object{
            ...
            if ($InvalidPkgLibPackages.Count -eq 0){
                ...
                if ($InvalidPkgLibPackages.Count -gt $WarnThreshold) {
                    ...
                    $response = Suspend-Script -Message "Proceed with removing invalid PkgLib packages on $($Env:COMPUTERNAME)?" -Warning -Buttons 4
                } else {
                ...
                if ($response -ieq "No") {
                    ...
                    $InvalidPkgLibPackages | ForEach-Object {
                        $InvalidPackageID = $_.PackageID
            ...
            if ($missingPkgLibPackages.Count -gt 0) {
                ...
                $null = Suspend-Script -Message "Please manually redistribute above packages to $Env:COMPUTERNAME." -Buttons 0
            }
            Write-Host ""
        }
    }
