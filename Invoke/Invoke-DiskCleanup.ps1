...
param(
  ...
  <#  $Before = Get-WmiObject Win32_LogicalDisk | Where-Object { $_.DriveType -eq "3" } | Select-Object SystemName,
    ...
    @{ Name = "PercentFree" ; Expression = {"{0:P1}" -f ( $_.FreeSpace / $_.Size ) } } |
        ...
        Out-String
   #>
   $Before = Get-WmiObject Win32_LogicalDisk | Where-Object { $_.DriveType -eq "3" -and $_.DeviceID -eq "C:"} | Select-Object SystemName,
    @{ Name = "Drive" ; Expression = { ( $_.DeviceID ) } },
    @{ Name = "FreeSpace (GB)" ; Expression = {"{0:N1}" -f ( $_.Freespace / 1gb ) } }| Out-String
...
#>
    ...
    $After1 = Get-WmiObject Win32_LogicalDisk| Where-Object { $_.DriveType -eq "3" -and $_.DeviceID -eq "C:"} #|($_.Freespace ) # Select-Object @{ Name = "Freespace"; Expression = {"{0:N1}" -f ( $_.Freespace / 1gb ) } }#|format-table -HideTableHeaders
    $After = [math]::Round(($After1.Freespace /1gb),2)


    ## Restarts wuauserv
    Get-Service -Name wuauserv | Start-Service -ErrorAction SilentlyContinue -WarningAction SilentlyContinue
   ...
   # Write-Host (Get-Date | Select-Object -ExpandProperty DateTime) -ForegroundColor Green
    ...
    Function PromptforScan {
        ...
        switch ($prompt) {
            ...
            1 {
                Write-Host "The Administrator chose to not scan $ScanPath for large files." -ForegroundColor DarkYellow 
            }
        }
    }
